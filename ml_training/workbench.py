...
def _run_review_interface(
    ...
    """
    Selects and runs the appropriate review user interface (TUI or classic) for the given review queue and runtime configuration.
    
    Chooses the configured UI mode; if Textual TUI mode is selected attempts to run the full-screen TUI and, on initialization failure, logs a warning and falls back to the classic non-TUI review loop.
    ...
    Returns:
        ReviewSessionMetrics: Aggregated metrics and staged labels produced by the completed review session.
    """
...
def _evaluate_command(args: argparse.Namespace) -> int:
    """
    Execute the project's evaluation script with the specified model and evaluation data.
    
    Parameters:
        args (argparse.Namespace): Parsed CLI args. Relevant attributes:
            - model: path to the trained model to evaluate.
            - data: optional path to evaluation CSV; when omitted the function uses the module's default evaluation data.
            - label_column: optional name of the ground-truth label column to pass through.
            - hybrid_threshold: optional numeric threshold to pass through for hybrid prediction logic.
...
def _resolve_eval_data_for_run(args: argparse.Namespace) -> Path:
    """
    Select the filesystem path to use for evaluation data based on the provided CLI arguments.
    
    Parameters:
        args (argparse.Namespace): Parsed CLI arguments with attributes `eval_data`, `skip_split`, `prepared_data`, and `unseen_data`.
    
    Returns:
        Path: Absolute Path to the chosen evaluation dataset. Prefers `args.eval_data` when provided, otherwise uses `args.prepared_data` if `skip_split` is true, and falls back to `args.unseen_data`.
    """
...
def _print_next_review_step(model_path: Path, data_path: Path) -> None:
    """
    Prints a suggested CLI command to launch the review stage using the given model and data paths.
    
    Parameters:
        model_path (Path): Filesystem path to the trained model to use in the suggested review command.
...
def _run_command_chain(args: argparse.Namespace) -> int:
    """
    Run training followed by optional evaluation, then print the suggested next review command.
    
    Parameters:
        args (argparse.Namespace): Parsed CLI options that control training, evaluation, and flow control (for example, model/data paths and the skip-evaluate flag).
    
    Returns:
        int: 0 on success, or the first non-zero exit code returned by the training or evaluation step.
    """
...
def _print_cli_overview() -> None:
    """
    Print a concise interactive overview of available CLI commands, built-in default paths, and the recommended workflow.
    
    Displays:
    - A "Commands" table listing top-level commands, their purpose, and an example invocation.
    - A "Built-In Paths" table showing default filesystem paths (model, review labels, seen/unseen splits, remaining unseen eval, and airway review set).
    - A "Workflow" panel with the recommended sequence of steps for iterative training and review.
...
def _parser_epilog() -> str:
    ...
    Returns:
        str: Multi-line help epilog containing example commands for initial model build, review + override loop, airway/anesthesia review set creation, and model evaluation.
    """
    return (
        "Workflows:\n"
        "  Initial model build:\n"
...
def _add_shared_training_arguments(parser: argparse.ArgumentParser) -> None:
    """
    Register common training-related command-line options on the provided ArgumentParser.
    
    Parameters:
        parser (argparse.ArgumentParser): The argument parser to extend with shared training options such as dataset and model paths, sampling and split parameters, worker and labeling settings, and common boolean flags (skip-prepare, skip-split, skip-evaluate, cross-validate, force).
    """
...
def _add_eval_label_argument(parser: argparse.ArgumentParser) -> None:
    ...
    - `--hybrid-threshold`: floating-point decision threshold forwarded to evaluation (default shown in help).
    """
    parser.add_argument(
        "--eval-label-column",
        default=None,
...
def _configure_train_parser(subparsers: argparse._SubParsersAction) -> None:
    """
    Register the "train" subcommand with the top-level argument parser and attach shared training options.
    
    Parameters:
        subparsers (argparse._SubParsersAction): The subparsers object returned by ArgumentParser.add_subparsers() into which the "train" command will be registered.
        
    Description:
        The created "train" command runs deterministic training stages (by default: prepare, split, train, and evaluate) and exposes the shared training-related arguments added by _add_shared_training_arguments. Example usages include:
...
def _configure_evaluate_parser(subparsers: argparse._SubParsersAction) -> None:
    """
    Register and configure the "evaluate" subcommand on the given argparse subparsers object.
    
    Adds an "evaluate" parser and the following options:
    - --model: path to the model to evaluate (defaults to DEFAULT_MODEL_PATH).
    - --data: optional evaluation CSV path; when omitted the built-in default unseen evaluation paths are used.
    - --label-column: optional ground-truth label column name (auto-detected when omitted).
    - --hybrid-threshold: float threshold used when reporting labeled accuracy (defaults to DEFAULT_ML_THRESHOLD).
    
    Parameters:
        subparsers (argparse._SubParsersAction): The argparse subparsers collection to which the
            "evaluate" command parser will be added.
    """
...
def _configure_review_parser(subparsers: argparse._SubParsersAction) -> None:
    """
    Register the "review" subcommand and its command-line arguments on the provided subparsers.
    
    Configures the review command used to run the interactive review workflow and adds arguments for model/data paths, selection focus, confidence threshold, maximum cases, output/progress files, resume behavior, and UI mode.
    
    Parameters:
        subparsers (argparse._SubParsersAction): The parser group to which the "review" subcommand will be attached.
    """
...
def _configure_run_parser(subparsers: argparse._SubParsersAction) -> None:
    """
    Register the "run" subcommand parser which configures a one-command pipeline for prepare/split/train/evaluate.
    
    Adds shared training arguments, evaluation-related options, and help/epilog text for the "run" command:
    - common training flags via _add_shared_training_arguments
    - --eval-data to optionally specify an evaluation dataset (defaults to the unseen split when omitted)
    - evaluation label and hybrid-threshold flags via _add_eval_label_argument
    
    Parameters:
        subparsers (argparse._SubParsersAction): The subparsers object to which the "run" parser will be added.
    """
...
def _configure_airway_review_set_parser(
    ...
    """
    Register the "airway-review-set" CLI subcommand and its arguments on the provided subparsers.
    
    Parameters:
        subparsers (argparse._SubParsersAction): The subparsers object returned by ArgumentParser.add_subparsers() to which the new subcommand will be attached.
    
    Description:
        Adds the "airway-review-set" command which scans the supervised CSV corpus and builds a focused manual-review CSV for airway/anesthesia targets. The command exposes these options:
          --base-dir: base directory containing supervised CSVs (default: PROJECT_ROOT / "Output-Supervised")
          --output: output CSV path for review candidates (default: DEFAULT_AIRWAY_REVIEW_OUTPUT)
          --max-cases: maximum number of candidate cases to include (default: 600)
          --default-year: default year to use when inferring missing year values (default: 2025)
    """
...
def _configure_retrain_parser(subparsers: argparse._SubParsersAction) -> None:
    """
    Configure the `retrain` subcommand parser with arguments used to merge review overrides, prepare retrain/eval datasets, and control retraining/evaluation behavior.
    
    Parameters:
        subparsers (argparse._SubParsersAction): The collection of subparsers returned by ArgumentParser.add_subparsers(); this function registers the `retrain` subparser and its options on it.
    """
...
def build_parser() -> argparse.ArgumentParser:
    ...
    Returns:
        argparse.ArgumentParser: Configured parser with subcommands: train, evaluate, review, run, airway-review-set, and retrain.
    """
...
def main() -> int:
    """
    Parse command-line arguments, print the CLI overview when no arguments are given, and dispatch the selected subcommand.
    
    When invoked with no subcommand prints an overview and detailed help. Otherwise looks up and calls the corresponding command handler and returns its exit code.
    
    Returns:
        An integer exit code: `0` on success, `1` if an unknown command is provided, or the exit code returned by the selected subcommand.
    """
    parser = build_parser()
